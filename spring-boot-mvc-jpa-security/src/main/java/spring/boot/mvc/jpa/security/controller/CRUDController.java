...
import spring.boot.mvc.jpa.security.form.model.UsuarioForm;
import spring.boot.mvc.jpa.security.model.Usuario;
import spring.boot.mvc.jpa.security.repository.UsuarioRepository;
...
public class CRUDController {
	...
	public ModelAndView salvar(@Valid @ModelAttribute("command") UsuarioForm form, BindingResult result,
			Map<String, Object> map) {

		if (result.hasErrors()) {
			map.put("mensagem",
					message.getMessage("spring.boot.controller.save.msg.campos", null, Locale.getDefault()));
			map.put("usuarios", this.getAll());
	...
	public ModelAndView getUser(@RequestParam("nome") String nome, Map<String, Object> map) {

		Usuario usuario = this.getUsuarioPorNome(nome);

		if (usuario == null) {
			map.put("mensagem", message.getMessage("spring.boot.controller.getuser.msg.campos", new Object[] { nome },
					Locale.getDefault()));
			return this.init(map);
		}
	...
	public ModelAndView delete(@RequestParam("nome") String nome, Map<String, Object> map) {

		Usuario usuario = this.getUsuarioPorNome(nome);

		if (usuario == null) {
			map.put("mensagem", message.getMessage("spring.boot.controller.remove.msg.campos", new Object[] { nome },
					Locale.getDefault()));
			return this.init(map);
		}
