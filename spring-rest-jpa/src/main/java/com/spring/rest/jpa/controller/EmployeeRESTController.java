...
public class EmployeeRESTController {
	...
	public @ResponseBody List<EmployeeVO> getAllEmployees() {
		...
		List<EmployeeVO> lista = null;

		try {
			lista = (List<EmployeeVO>) eRepository.findAll();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public @ResponseBody List<EmployeeVO> saveEmployee(
			@Validated @RequestBody EmployeeVO employee) {
		try {
			// Verifica a existencia do usuario, por nome
			EmployeeVO employeeExistente = eRepository
					.findByFirstNameContaining(employee.getFirstName());

			if (employeeExistente != null) {
				employee.setId(employeeExistente.getId());
			}
		...
		List<EmployeeVO> lista = this.getAllEmployees();
	...
	public @ResponseBody EmployeeVO getEmployee(@RequestBody EmployeeVO employee) {
		try {
			employee = eRepository.findOne(employee.getId());
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public @ResponseBody List<EmployeeVO> removeEmployee(
	...
	@RequestMapping(value="/employeerest.wadl" ,method = RequestMethod.GET, produces = { "application/xml" })
	public @ResponseBody Application generateWadl(HttpServletRequest request) {
		Application result = new Application();
		Doc doc = new Doc();
		doc.setTitle("spring-rest-jpa wadl");
